...
instance (PLC.ReadKnownIn uni (UPLC.Term name uni fun ()) Integer) => UnliftU name uni fun BuiltinInteger where
    unliftU t = toBuiltin <$> unliftUBuiltin @Integer t

instance (PLC.ReadKnownIn uni (UPLC.Term name uni fun ()) Data) => UnliftU name uni fun BuiltinData where
    unliftU t = dataToBuiltinData <$> unliftUBuiltin t

instance (PLC.ReadKnownIn uni (UPLC.Term name uni fun ()) BS.ByteString) => UnliftU name uni fun BuiltinByteString where
    unliftU t = toBuiltin <$> unliftUBuiltin @BS.ByteString t

instance (PLC.ReadKnownIn uni (UPLC.Term name uni fun ()) Text.Text) => UnliftU name uni fun BuiltinString where
    unliftU t = toBuiltin <$> unliftUBuiltin @Text.Text t

-- FIXME
--instance (ToBuiltin a arep, PLC.ReadKnownIn uni (UPLC.Term UPLC.Name uni fun ()) [a]) => UnliftU uni fun (BuiltinList arep) where
