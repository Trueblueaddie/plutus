...
constructorCaseArgTypes :: VarDecl tyname name uni a -> [Type tyname uni a]
constructorCaseArgTypes vd = funTyArgs (_varDeclType vd)

constructorArgsType :: a -> VarDecl tyname name uni a -> Type tyname uni a
...
mkSumTy :: MonadQuote m => ann -> Datatype TyName Name uni ann -> m (Type TyName uni ann)
...
mkBoundDatatypePatternFunctor :: MonadQuote m => ann -> Datatype TyName Name uni ann -> m (Type TyName uni ann)
...
-- @
mkDestructorTy :: MonadQuote m => Provenance a -> Datatype TyName Name uni (Provenance a) -> m (PIRType uni a)
mkDestructorTy ann dt@(Datatype _ _ tvs _ _) = do
    ...
    -- forall t_1 .. t_n
    let destrTy = PIR.mkIterTyForall tvs $ TyFun ann appliedAbstract scottTy
    pure $ fmap (\a -> DatatypeComponent DestructorType a) destrTy
